...
BCFM.model.selection(
  ...
  burnin = 10,            # Burnin for BIC calculation (use 10000+ for real analysis)
...
> **Output Directory:** The `output_dir` parameter specifies where model results will be saved. This ensures you have write permissions and can easily organize results from multiple analyses. If not specified, results are saved to the current working directory.
...
# Load BIC results (if multiple models were fitted)
...
When testing multiple configurations (e.g., `grouplist = 2:4, factorlist = 2:4`), a `BIC.Rdata` file is created for model comparison.
...
2. **Output Examination** - Exploring result files and BIC matrices
